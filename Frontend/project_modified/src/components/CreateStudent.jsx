...
function CreateStudent({ token }) {
  ...
  return (
    <div style={{ padding: '28px 32px' }}>
      ...
      ══════════════════════════════════════════════ */}
      {tab === 'single' && (
        <div className="svec-form-panel" style={{ }}>
                      ...
                      <tr key={i}>
                        ...
                        {REQUIRED_COLS.map(c => (
                          <td key={c} style={{ fontFamily: c === 'username' ? 'var(--font-mono)' : 'inherit', fontSize: c === 'username' ? '0.83rem' : 'inherit' }}>
                            {row[c] || <span style={{ color: 'var(--text-secondary)', fontStyle: 'italic' }}>—</span>}
                          </td>
                        ))}
                      </tr>
                    ))}
                  ...
                  <tbody>
                    ...
                    {bulkRunning && parsed.slice(bulkResults.length).map((row, i) => (
                      <tr key={`pending-${i}`} style={{ opacity: 0.4 }}>
                        <td style={{ color: 'var(--text-secondary)' }}>{bulkResults.length + i + 1}</td>
                        <td style={{ fontFamily: 'var(--font-mono)', fontSize: '0.83rem' }}>{row.username}</td>
                        <td>{row.name}</td>
                        <td><span style={{ padding: '2px 10px', borderRadius: '99px', fontSize: '0.75rem', fontWeight: 700, background: 'var(--bg)', color: 'var(--text-secondary)', border: '1px solid var(--border)' }}>⏳ Pending</span></td>
                        <td>—</td>
                      </tr>
                    ))}
                  </tbody>
