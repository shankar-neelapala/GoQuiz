...
function ExamSchedule(props) {
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      {schedule.length === 0 && (
        <div className="svec-no-exam">No exam schedule available</div>
      )}
      {schedule.length > 0 && result.map((items, index) => (
        !items[0]?.semester ? null : (
          <div key={index} style={{ marginBottom: '32px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
              ...
              <span style={{ fontSize: '0.78rem', fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--primary-dark)' }}>
                {items[0]?.semester} Semester
              </span>
              <span style={{ fontSize: '0.7rem', fontWeight: 600, color: 'var(--text-muted)', background: 'rgba(0,0,0,0.04)', padding: '2px 8px', borderRadius: '99px', border: '1px solid var(--border)' }}>
                {items[0]?.examtype}
              </span>
            </div>
                ...
                <tbody>
                  {items.map((item, rowIndex) => (
                    <tr key={item.id || rowIndex}>
                      <td style={{ color: 'var(--text-secondary)', fontWeight: 600 }}>{rowIndex + 1}</td>
                      <td style={{ fontWeight: 500 }}>{item.subject}</td>
                      <td>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.78rem', background: 'rgba(0,0,0,0.04)', color: 'var(--text-secondary)', padding: '2px 8px', borderRadius: '4px', border: '1px solid var(--border)' }}>
                          {item.coursecode}
                        </span>
                      </td>
                      <td>{item.date}</td>
