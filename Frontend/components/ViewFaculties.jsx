...
function ViewFaculties({ token }) {
  ...
  return (
    <div style={{ padding: '28px 32px' }}>
      <div className="svec-form-panel" style={{ maxWidth: '400px', marginBottom: '24px' }}>
              ...
              {Object.entries(editableFaculty).filter(([key]) => key !== 'image' && key !== 'subjectsname').map(([key, value]) => {
                ...
                return (
                  <div key={key} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      {isReadOnly ? (
                        <div style={{ padding: '8px 12px', background: 'var(--bg)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', fontSize: '0.88rem', color: 'var(--text-muted)' }}>{displayValue}</div>
                      ) : (
